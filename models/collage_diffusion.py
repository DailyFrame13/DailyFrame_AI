...
from models.layers import ImageLayer
from utils.masking_collage import generate_mask, apply_mask
...
class StableDiffusionControlNetPipeline(DiffusionPipeline, TextualInversionLoaderMixin):
    ...
    def collage(
        ...
        self.check_inputs(
            prompt,
            controlnet_images[0] if isinstance(self.controlnet, ControlNetModel) else controlnet_images,
            height,
            width,
            callback_steps,
            negative_prompt,
        ...
        if isinstance(self.controlnet, ControlNetModel):
            ...
            controlnet_images = controlnet_image  # 이후 forward에서 그대로 사용
        elif isinstance(self.controlnet, MultiControlNetModel):
    ...
    def preprocess_layers(
        ...
        for layer in layers:
            ...
            if url.startswith("file://"):
                ...
                layer_rgba = Image.open(local_path).convert("RGBA")
            else:
...
def load_collage_pipeline(
    device: str = "cuda",
    dtype: torch.dtype = torch.float16,
) -> StableDiffusionControlNetPipeline:
    # 1) Base SD 파이프라인 로드 (runwayml 1.5 예시)
    ...
    # 2) Canny ControlNet 로드
    controlnet = ControlNetModel.from_pretrained(
        "lllyasviel/sd-controlnet-canny",
        torch_dtype=dtype,
    )

    # 3) 네가 정의한 커스텀 파이프라인으로 래핑
